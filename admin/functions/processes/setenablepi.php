<?php
    ...
    foreach($Items as $item) {
        if(isset($_POST[$item["ItemId"]])) {
            $enabled = 1;
            $lastEnable = $db->fetchColumn('SELECT Enabled FROM PiPrices WHERE ItemId= :item AND Time= :time', array('item' => $item["ItemId"], 'time' => $lastUpdate));
            if($enabled != $lastEnable) {
                $db->update('PiPrices', array('ItemId' => $item["ItemId"], 'Time' => $lastUpdate), array('Enabled' => $enabled));
            }
        } else {
            $enabled = 0;
            $lastEnable = $db->fetchColumn('SELECT Enabled FROM PiPrices WHERE ItemId= :item AND Time= :time', array('item' => $item["ItemId"], 'time' => $lastUpdate));
