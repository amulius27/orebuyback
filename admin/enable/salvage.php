...
$Items = $db->fetchRowMany('SELECT Name,ItemId FROM ItemIds WHERE Grouping= :group', array('group' => 'Salvage'));

$lastUpdate = $db->fetchColumn('SELECT MAX(time) FROM SalvagePrices WHERE ItemId= :item', array('item' => $id));

foreach($Items as $item) {
    $itemEnabled[$item['ItemId']] = $db->fetchColumn('SELECT Enabled FROM SalvagePrices WHERE ItemId= :item', array('item' => $$item));
}
