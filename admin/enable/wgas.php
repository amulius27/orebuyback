...
$Items = $db->fetchRowMany('SELECT Name,ItemId FROM ItemIds WHERE Grouping= :group', array('group' => 'WGas'));

$lastUpdate = $db->fetchColumn('SELECT MAX(time) FROM WGasPrices WHERE ItemId= :item', array('item' => $id));

foreach($Items as $item) {
    $itemEnabled[$item['ItemId']] = $db->fetchColumn('SELECT Enabled FROM WGasPrices WHERE ItemId= :item', array('item' => $$item));
}
    ...
    <div class="container">
        <div class="panel panel-default">
            <div class="panel-heading" align="center">
                <h3 class="panel-title"><span style="font-family: Arial; color: #FFF;"<strong>Enable WGas Pricing Form</strong></span><br></h3>
            </div>
