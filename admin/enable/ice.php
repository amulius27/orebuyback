...
$Items = $db->fetchRowMany('SELECT Name,ItemId FROM ItemIds WHERE Grouping= :group', array('group' => 'Ice'));

$lastUpdate = $db->fetchColumn('SELECT MAX(time) FROM OrePrices WHERE ItemId= :item', array('item' => $id));

foreach($Items as $item) {
    $itemEnabled[$item['ItemId']] = $db->fetchColumn('SELECT Enabled FROM OrePrices WHERE ItemId= :item', array('item' => $$item));
}
    ...
    <div class="container">
        <div class="panel panel-default">
            ...
            <div class="panel-body" align="center">
                <form action="/../processes/setenableice.php" method="POST">
                    <?php
                        foreach($Items as $item) {
